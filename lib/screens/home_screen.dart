...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<Map<String, String>> recentTranslations = [
  ...
  void _addTranslation(String text) {
    setState(() {
      ...
      // Agregar a traducciones recientes
      recentTranslations.insert(0, {
        "text": text,
        "time": "Ahora mismo"
      });

      // Mantener solo las últimas 5 traducciones
      if (recentTranslations.length > 5) {
        recentTranslations.removeLast();
      }

      // Actualizar los tiempos después de 1 minuto
      Future.delayed(const Duration(minutes: 1), () {
        if (mounted) {
          setState(() {
            for (var translation in recentTranslations) {
              if (translation["time"] == "Ahora mismo") {
                translation["time"] = "Hace 1 minuto";
              }
            }
          });
        }
  ...
  void _handleKeyEvent(RawKeyEvent event) {
    if (event is RawKeyDownEvent) {
      print('Tecla presionada: ${event.logicalKey}'); // Para debug
      switch (event.logicalKey) {
        case LogicalKeyboardKey.numpad1:
        case LogicalKeyboardKey.digit1:
          _addTranslation("Hola");
          break;
        case LogicalKeyboardKey.numpad2:
        case LogicalKeyboardKey.digit2:
          _addTranslation("Bien");
          break;
